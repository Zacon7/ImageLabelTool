...
class LabelSelector:
    def __init__(self, master, label_list, on_ok, on_cancel):
        ...
        for label in label_list:
            var = tkinter.IntVar()
            cb = tkinter.Checkbutton(self.top, text=label, variable=var, font=font.Font(size=12))
        ...
        cancel_button = tkinter.Button(self.top, text="取消", command=self.on_cancel_button)
...
class ImageLabelingTool:
    def __init__(self):
        ...
        self.rect_count = tkinter.Label(self.canvas_frame, text='(' + str(len(self.rect_stack)) + ' rectangles)',
                                        font=font.Font(size=10))
        self.rect_count.place(relx=0.43, rely=0.93, anchor='w')

        self.label_index = tkinter.Label(self.canvas_frame,
                                         text='[{}/{}]'.format(self.image_idx + 1, len(self.image_list)),
                                         font=font.Font(size=12))
        self.label_index.place(relx=0.46, rely=0.96, anchor='w')

        self.btn_pre = tkinter.Button(self.canvas_frame, text='上一张', command=lambda: self.prev_image(), width=7,
                                      height=1)
        self.btn_next = tkinter.Button(self.canvas_frame, text='下一张', command=lambda: self.next_image(), width=7,
                                       height=1)
    ...
    def show_image(self, image_name):
        ...
        self.img_canvas.create_image(self.canvas_width // 2, self.canvas_height // 2, image=self.photo)
    ...
    def restore_rect(self, image_idx):
        ...
        for rect_dict in self.json_dict[image_name]:  # 有多少个矩形框就有多少个rect_dict
            ...
            rect_coords = rect_dict["points"]  # tuple: ((start_x, start_y), (end_x, end_y)
            (start_x, start_y), (end_x, end_y) = rect_coords
            self.rect_start_x, self.rect_start_y = start_x + self.img_start_x, start_y + self.img_start_y
            self.rect_end_x, self.rect_end_y = end_x + self.img_start_x, end_y + self.img_start_y

            self.rect = self.img_canvas.create_rectangle(start_x + self.img_start_x, start_y + self.img_start_y,
                                                         end_x + self.img_start_x, end_y + self.img_start_y,
                                                         outline='yellow', width=2)
            self.tag_text = self.img_canvas.create_text(self.rect_start_x, self.rect_start_y - 20,
                                                        text=labels, anchor="w", font=font.Font(size=11), fill="yellow")
    ...
    def on_left_press(self, event):
        ...
        if (self.img_start_x <= self.rect_start_x <= self.img_end_x and
                self.img_start_y <= self.rect_start_y <= self.img_end_y):
            self.is_drawing_rect = True
            self.rect = self.img_canvas.create_rectangle(self.rect_start_x, self.rect_start_y,
    ...
    def on_left_move(self, event):
        ...
        if self.is_drawing_rect and \
                (self.img_start_x <= self.rect_end_x <= self.img_end_x and
                 self.img_start_y <= self.rect_end_y <= self.img_end_y):
            self.img_canvas.coords(self.rect, self.rect_start_x, self.rect_start_y, self.rect_end_x, self.rect_end_y)
    ...
    def show_label_selector(self):
        self.label_selector = LabelSelector(self.root, self.label_list, self.on_button_ok, self.on_button_cancel)
    ...
    def on_button_ok(self, tags):
        ...
        self.json_dict[image_name].append({
            "labels": labels,
            "points": ((self.rect_start_x - self.img_start_x, self.rect_start_y - self.img_start_y),
                       (self.rect_end_x - self.img_start_x, self.rect_end_y - self.img_start_y))
        })
        self.rect_stack.append(self.rect)
        self.tags_stack.append(self.tag_text)
        self.rect_count.configure(text='(' + str(len(self.rect_stack)) + ' rectangles)')
