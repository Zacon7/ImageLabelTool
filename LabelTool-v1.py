...
class ImageLabelingTool:
    def __init__(self):
        ...
        self.rect_count = tkinter.Label(
            self.canvas_frame,
            text="(" + str(len(self.rect_list)) + " rectangles)",
    ...
    def init_checkbuttons(self):
        ...
        for label_name in self.label_list:
            ...
            cb = tkinter.Checkbutton(
                ...
                onvalue=1,
                offvalue=0,
                command=lambda name=label_name, var=is_check: self.update_dict_cb(
                    name, var.get()
                ),
            )
            cb.pack(anchor="w")
    ...
    def on_left_press(self, event):
        ...
        if (
            self.start_x >= self.img_start_x
            and self.start_x <= self.img_end_x
            and self.start_y >= self.img_start_y
            and self.start_y <= self.img_end_y
        ):
    ...
    def on_left_move(self, event):
        ...
        if self.is_drawing_rect and (
            self.end_x >= self.img_start_x
            and self.end_x <= self.img_end_x
            and self.end_y >= self.img_start_y
            and self.end_y <= self.img_end_y
        ):
    ...
    def on_left_release(self, event):
        ...
        if self.is_drawing_rect and (
            ...
            image_name = self.image_list[self.image_idx]
            self.json_dict[image_name][1].append(
                (
                    (self.start_x - self.img_start_x, self.start_y - self.img_start_y),
